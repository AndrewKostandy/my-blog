...
Businesses face various types of problems that require making optimal decisions in order to achieve a certain objective. In this post, I will discuss a supply source & route selection problem scenario and how network modeling with linear programming can be used to identify the optimal decisions that would minimize the cost of shipping. I will use Gurobi Python to model & solve this problem.
...
```{r, echo=FALSE}
t <- tribble(
  ~Shops,           ~Boston,   ~Miami, ~Seattle,          ~`Los Angeles`,           ~Demand,
  "Minneapolis",      "$20",    "$24",    "$22",        "$26",            "4500",
  "Chicago",          "$18",    "$20",    "$23",        "$24",            "7000",
  "Denver",           "$24",    "$25",    "$16",        "$15",            "4500",
  "Dallas",           "$23",    "$17",    "$23",        "$17",            "5500",
  "Production",      "3500",    "5000",   "4000",      "6000",                ""
)
```

```{r, echo=FALSE}
kable(t, align = "lrrrrr") %>% row_spec(5, bold = TRUE) %>% add_header_above(c(" " = 1, "Factory Options" = 4, " " = 1))
```
...
![](/post/2018-06-19-source-route-selection-with-linear-programming_files/network_plot.png)
...
Let's add all those constraints to our Gurobi Python model:
...
![](/post/2018-06-19-source-route-selection-with-linear-programming_files/network_plot5.png)
